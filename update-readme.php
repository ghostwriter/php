...
{
    return sprintf(
        '### ![Terminal](resource/icons/terminal.svg) Pull & Run `PHP %s` %s image from the command line',
        $phpVersion,
        $type
    ) . PHP_EOL;
}
...
{
    return sprintf(
        '### ![Code](resource/icons/code.svg) Use `PHP %s` %s image in Dockerfile',
        $phpVersion,
        $type
    ) . PHP_EOL;
}
...
{
    return sprintf('## PHP %s : %s', $phpVersion, mb_strtoupper($type)) . PHP_EOL;
}
